...
`);
  ...
  app.post('/api/ai', async (req, res) => {
    ...
    if (!apiKey) {
      // APIキー未設定時: feature ごとに異なるフォールバック応答を返す
      const fallbacks = {
        'doc_稟議書':   'ご指定の「稟議書」のたたき台を作成しました。UMA BizFlow導入に関する目的・費用・期待効果・リスクを盛り込んでいます。内容を確認の上、承認者へ提出してください。',
        'doc_月次報告書':'ご指定の「月次報告書」のたたき台を作成しました。2026年4月の売上・顧客数・ワークフロー処理状況・課題・来月方針をまとめています。数値を実績値に差し替えてご利用ください。',
        'doc_議事録':   'ご指定の「議事録」のたたき台を作成しました。主要な決定事項とネクストアクションをまとめています。担当者・期日を確認の上、配布してください。',
        doc:            'ご指定の文書のたたき台を作成しました。主要な決定事項とネクストアクションをまとめています。',
        analytics:      '現在の業務データを分析しました。売上は前月比+12%で推移しており、未回収売掛金の早期回収が優先課題です。',
        contract:       '契約書のリスク診断が完了しました。支払い条件および免責事項において、貴社に不利になる可能性のある条項が2箇所見つかりました。',
        support:        'システム操作に関するご質問ですね。請求書登録の手順について、ガイドを生成しました。こちらをご確認ください。',
        quick_summary:  '今月の業務処理状況を確認しました。受注・請求・入金の各プロセスは概ね順調で、ワークフロー承認率は85%です。未処理の申請が3件あります。',
        quick_delay:    '承認が遅れている案件が2件あります。DX推進支援（テクノフロンティア）が5日超過、製造ライン管理（山田製作所）が3日超過です。本日中に承認者へリマインドを推奨します。',
        quick_improve:  '改善優先課題は3点です。①未回収売掛金の督促フロー自動化 ②経費精算の承認SLA設定（目標2営業日）③既存顧客へのサブスク提案で売上の安定化を図ることを推奨します。',
      };
      const text = fallbacks[feature] ||
        'UMA BizFlow AIアシスタントです。業務データを分析しました。売上は前月比+12%で好調に推移しています。未回収売掛金の早期回収と費用率の最適化が優先課題です。詳しい分析が必要な項目をお知らせください。';
